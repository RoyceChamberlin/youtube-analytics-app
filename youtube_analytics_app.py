...
st.set_page_config(page_title="Chamberlin Media Monitor", layout="wide", initial_sidebar_state="expanded")
...
<style>
    ...
    h1 { color: #ffffff; font-weight: 600; font-size: 28px; }
    h2, h3 { color: #ffffff; }
...
st.title("Chamberlin Media Monitor")
...
if st.session_state.channels:
    ...
    for name in list(st.session_state.channels.keys()):
        ...
        if col2.button("Remove", key=f"rm_{name}"):
            ...
            st.session_state.notes.pop(name, None)
            st.session_state.generated_ideas.pop(name, None)
            st.rerun()
...
st.caption("Chamberlin Media Monitor • Built to help your team improve video performance")
